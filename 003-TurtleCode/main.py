...
class TurtleCode:
    ...
    def __init__(self):
        ...
        self.main_text_box = Text(self.window, font=self.default_font, yscrollcommand=self.text_box_scrollbar.set)
        ...
        self.autofill_listbox = Listbox(self.autofill_size_frame)
        ...
        self.tree_style.configure("Treeview",
                                  background=self.config["project_view"]["bg_col"]
                                  )
    ...
    def set_tags(self, flag):
        ...
        if flag == "all":
            start = 0
            end = len(self.file_text.split("\n"))
        elif flag == "section":
            line_num = int(self.main_text_box.index(INSERT).split(".")[0])-1
            start = max(0, line_num-self.main_text_box_height)
            end = min(len(self.file_text.split("\n"))-2, line_num+self.main_text_box_height)
